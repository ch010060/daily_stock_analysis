...
_CHINESE_SECTION_PATTERNS = {
    ...
    "news_catalysts": r"###\s*五、(?:訊息催化|後市展望)",
}
...
class MarketAnalyzer:
    ...
    def _get_turnover_unit_label(self) -> str:
        ...
        if self.region == "tw":
            return "TWD bn" if self._get_review_language() == "en" else "億元臺幣"
...
"""
    ...
    def search_market_news(self) -> List[Dict]:
        ...
        try:
            ...
            market_names = {"cn": "大盤", "us": "US market", "hk": "HK market"}
    ...
    def _build_stats_block(self, overview: MarketOverview) -> str:
        ...
        lines = [
            f"- **盤面訊號**：{score}/100（{label}，{light['label']}）",
            f"- **訊號依據**：{'；'.join(light['reasons'])}",
    ...
    def build_market_light_snapshot(self, overview: MarketOverview) -> Dict[str, Any]:
        ...
        if self._get_review_language() == "en":
            ...
            guidance_map = {
                "green": "風險偏好尚可，關注主線延續與部位紀律。",
                "yellow": "訊號分化，控制部位並等待量價確認。",
    ...
    def _build_market_light_reasons_zh(self, overview: MarketOverview, score: int) -> List[str]:
        ...
        if not reasons:
            reasons.append("結構化漲跌資料有限，按可用行情綜合判斷")
        return reasons[:4]
...
領跌: {bottom_sectors_text if bottom_sectors_text else "暫無資料"}"""
        ...
        if review_language == "en":
            ...
            indices_placeholder = indices_text if indices_text else "暫無指數資料（介面異常）"
...
"""
        ...
        # A 股場景使用中文提示語
        return f"""你是一位專業的A/H/美股市場分析師，請根據以下資料生成一份結構化的{self._get_market_scope_name('zh')}大盤覆盤報告。
...
- 報告要像交易員盤後工作臺：先給結論，再按資料表、主線、催化、計劃展開
...
### 一、盤面總覽
{dashboard_block or "暫無市場寬度資料。"}

### 二、指數結構
{indices_block or indices_text or "暫無指數資料。"}

### 三、板塊主線
{sector_block or "- 暫無板塊漲跌榜資料。"}
...
### 六、明日交易計劃
- **結論**：均衡觀察。
- **部位**：控制在中性區間，等待指數與主線共振。
- **關注方向**：{top_text or "強於指數的主線板塊"}。
- **迴避方向**：{bottom_text or "連續走弱且缺少修復訊號的方向"}。
