...
class BacktestService:
        ...
        *,
        ...
        for analysis in candidates:
            ...
            try:
                ...
                errors += 1
                logger.error(f"回測失敗: {analysis.code}#{analysis.id}: {exc}")
                results_to_save.append(
                    BacktestResult(
                        analysis_history_id=analysis.id,
                        code=analysis.code,
        ...
        *,
        ...
        for window in requested_windows:
            try:
                ...
                logger.error("多視窗回測失敗: T+%s: %s", window, exc)
                window_results[window] = {
                    "window": window,
                    "status": "error",
                    "error": str(exc),
    ...
    def _resolve_analysis_date(self, analysis) -> Optional[date]:
        ...
        logger.warning(f"無法確定分析日期，跳過記錄: {analysis.code}#{getattr(analysis, 'id', '?')}")
        return None
    ...
    def _try_fill_daily_data(self, *, code: str, analysis_date: date, eval_window_days: int) -> None:
        try:
            ...
            logger.warning(f"補全日線資料失敗({code}): {exc}")
