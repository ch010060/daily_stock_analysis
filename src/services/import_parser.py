...
def parse_import_from_bytes(data: bytes, filename: Optional[str] = None) -> List[Tuple[Optional[str], Optional[str], str]]:
    ...
    if len(data) > MAX_FILE_BYTES:
        raise ValueError(f"檔案超過 {MAX_FILE_BYTES // (1024 * 1024)}MB 限制")

    ext = ""
    if filename:
        ext = "." + filename.rsplit(".", 1)[-1].lower() if "." in filename else ""
    logger.debug(f"[ImportParser] 開始解析檔案: filename={filename or '-'}, ext={ext or '-'}, bytes={len(data)}")
    ...
    if ext == ".xlsx" or looks_like_zip:
        try:
            ...
            if looks_like_zip:
                hint = (
                    "請確認：(1) 檔案為 .xlsx 格式；(2) 工作表不為空；(3) 檔案未損壞。"
            ...
            logger.warning(f"副檔名為 .xlsx 但未解析為 Excel，將回退文字解析: {e}")

    # .xls not supported
    if ext == ".xls":
        raise ValueError("僅支援 .xlsx 格式，請將 .xls 另存為 .xlsx 後重試")
...
def parse_import_from_text(text: str) -> List[Tuple[Optional[str], Optional[str], str]]:
    ...
    if len(text.encode("utf-8")) > MAX_TEXT_BYTES:
        raise ValueError(f"文字超過 {MAX_TEXT_BYTES // 1024}KB 限制")

    logger.debug(f"[ImportParser] 開始解析貼上文字: bytes={len(text.encode('utf-8'))}")
