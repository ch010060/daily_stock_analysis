    ...
    if left and not right:
        target.append(
            _issue(
                ...
                f"{channel_name} 已配置 {left_key}，但缺少 {right_key}，該通道不會啟用。",
                key=right_key,
            )
        )
    if right and not left:
        target.append(
            _issue(
                severity,
                "partial_channel_config",
                f"{channel_name} 已配置 {right_key}，但缺少 {left_key}，該通道不會啟用。",
                key=left_key,
            )
        )
...
def run_notification_diagnostics(config: Config) -> NotificationDiagnosticResult:
    ...
    info: List[NotificationDiagnosticIssue] = [
        _issue(
            ...
            "釘釘會話和飛書會話屬於執行時訊息上下文通道，無法僅靠靜態 .env 完整判斷。",
        ),
        _issue(
            "info",
            "phase_scope",
            "通知診斷會檢查通道基線、只讀診斷、Web 測試、P3 路由配置、P4 降噪配置和 P6 ntfy/Gotify 通道。",
        ),
    ]
    ...
    if not configured:
        errors.append(
            _issue(
                ...
                "0 個通知通道已配置；如需傳送通知，請至少配置一個通道的 minimal key。",
            )
        )
    ...
    if (_has(config, "feishu_webhook_secret") or _has(config, "feishu_webhook_keyword")) and not _has(config, "feishu_webhook_url"):
        warnings.append(
            _issue(
                ...
                "已配置飛書 Webhook 高階安全項，但缺少 FEISHU_WEBHOOK_URL，飛書 Webhook 通道不會啟用。",
    ...
    if _has(config, "pushplus_topic") and not _has(config, "pushplus_token"):
        warnings.append(
            _issue(
                "warning",
                "advanced_without_minimal",
                "已配置 PUSHPLUS_TOPIC，但缺少 PUSHPLUS_TOKEN，PushPlus 通道不會啟用。",
                key="PUSHPLUS_TOKEN",
            )
        )
    if _has(config, "ntfy_token") and not _has(config, "ntfy_url"):
        warnings.append(
            _issue(
                "warning",
                "advanced_without_minimal",
                "已配置 NTFY_TOKEN，但缺少 NTFY_URL，ntfy 通道不會啟用。",
                key="NTFY_URL",
            )
        )
    if (
        ...
        warnings.append(
            _issue(
                "warning",
                "advanced_without_minimal",
                "已配置自定義 Webhook 高階項，但缺少 CUSTOM_WEBHOOK_URLS，自定義 Webhook 通道不會啟用。",
    ...
    if _has(config, "astrbot_token") and not _has(config, "astrbot_url"):
        warnings.append(
            _issue(
                "warning",
                "advanced_without_minimal",
                "已配置 ASTRBOT_TOKEN，但缺少 ASTRBOT_URL，AstrBot 通道不會啟用。",
                key="ASTRBOT_URL",
            )
        )
        ...
        if invalid_channels:
            errors.append(
                _issue(
                    ...
                    (
                        f"{route_config['env_key']} 包含未知通知通道: {', '.join(invalid_channels)}；"
        ...
        if disabled_channels:
            warnings.append(
                _issue(
                    ...
                    (
                        f"{route_config['env_key']} 路由 {route_type} 指向未啟用通道: "
                        f"{', '.join(disabled_channels)}；這些通道不會收到該型別通知。"
                    ),
                    key=route_config["env_key"],
                )
            )
...
def format_notification_diagnostics(result: NotificationDiagnosticResult) -> str:
    ...
    lines = [
        "通知配置診斷",
        f"已配置通道: {len(result.configured_channels)} 個",
    ]
    if result.configured_channels:
        ...
        lines.append("通道列表: " + ", ".join(channel_names))
    else:
        lines.append("通道列表: (無)")
