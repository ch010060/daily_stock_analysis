...
class TelegramSender:
    ...
    def _send_plain_text_fallback(
        ...
        *,
        ...
        logger.info("Telegram Markdown 解析失敗，嘗試使用純文字格式重新傳送...")
        ...
        if response.status_code == 200:
            try:
                ...
                logger.error("Telegram 純文字回退失敗: 響應不是有效 JSON")
                logger.error(f"響應內容: {response.text}")
                return False
            ...
            logger.error("Telegram 純文字回退失敗: Telegram API 返回 ok=false")
            logger.error(f"響應內容: {response.text}")
            return False

        logger.error(f"Telegram 純文字回退失敗: HTTP {response.status_code}")
        logger.error(f"響應內容: {response.text}")
        return False
