...
class ReportLanguageTestCase(unittest.TestCase):
    def test_get_signal_level_handles_compound_sell_advice(self) -> None:
        signal_text, emoji, signal_tag = get_signal_level("賣出/觀望", 60, "zh")
    ...
    def test_get_sentiment_label_preserves_higher_band_thresholds(self) -> None:
        ...
        self.assertEqual(get_sentiment_label(20, "zh"), "悲觀")

    def test_localize_trend_prediction_preserves_fine_grain_zh_states(self) -> None:
        self.assertEqual(localize_trend_prediction("多頭排列", "zh"), "多頭排列")
        self.assertEqual(localize_trend_prediction("弱勢空頭", "zh"), "弱勢空頭")
    ...
    def test_localize_trend_prediction_still_translates_english_input_for_zh(self) -> None:
        ...
        self.assertEqual(localize_trend_prediction("very bearish", "zh"), "強烈看空")
    ...
    def test_infer_decision_type_from_advice_matches_chinese_phrases(self) -> None:
        ...
        self.assertEqual(infer_decision_type_from_advice("建議減倉"), "sell")
        self.assertEqual(infer_decision_type_from_advice("繼續持有"), "hold")
        self.assertEqual(infer_decision_type_from_advice("建議洗盤觀察"), "hold")
        self.assertEqual(infer_decision_type_from_advice("洗盤觀察", default=""), "hold")
        self.assertEqual(infer_decision_type_from_advice("觀察", default=""), "hold")
        self.assertEqual(infer_decision_type_from_advice("不建議買進"), "hold")
        self.assertEqual(
            infer_decision_type_from_advice("當前不跌破支撐位繼續持有"),
            "hold",
        )
        self.assertEqual(
            infer_decision_type_from_advice("不破支撐後仍可持有"),
            "hold",
        )
