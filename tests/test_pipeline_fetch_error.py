...
class PipelineFetchErrorTestCase(unittest.TestCase):
    ...
    def test_fetch_and_save_uses_effective_trading_date_for_resume_check(self, _mock_target):
        ...
        _mock_target.assert_called_once_with("2330", current_time=current_time)
        pipeline.db.has_today_data.assert_called_once_with("2330", date(2026, 3, 27))
        pipeline.fetcher_manager.get_daily_data.assert_not_called()

    def test_resolve_resume_target_date_normalizes_supported_tw_us_formats(self):
        with patch("src.core.pipeline.get_market_for_stock", return_value="tw") as mock_market, patch(
            ...
            for code in ("2330.TW", "TW:2330", "00981A.TW"):
