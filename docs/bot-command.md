...
- `/status` 顯示的 LLM 可用性遵循系統統一執行時優先順序：
  - `LITELLM_CONFIG`（LiteLLM YAML）
  - `LLM_CHANNELS`
  - legacy provider 鍵（`GEMINI_API_KEY` / `OPENAI_API_KEY` / `ANTHROPIC_API_KEY` / `DEEPSEEK_API_KEY`）
- 當主模型（`LITELLM_MODEL` 或 `AGENT_LITELLM_MODEL`）在當前啟用層無可用來源時，會展示“AI 服務未配置”，並保留使用者可見原因行。
- 本倉庫 `requirements.txt` 的執行時依賴約束為 `litellm>=1.80.10,!=1.82.7,!=1.82.8,<2.0.0`，該約束內本鏈路以現有相容行為為準。
- 該診斷規則與 `GET /api/v1/system/config/setup/status` 的 LLM 檢查保持一致：`LITELLM_CONFIG`/`LLM_CHANNELS` 為高優先順序；模式切換時不會做靜默遷移，切回舊模式由使用者顯式恢復歷史值或回滾。

### 回退與遷移邊界

- `LITELLM_CONFIG` 與 `LLM_CHANNELS` 任一生效時，下層 legacy 配置會被該層忽略（不會繼續作為本次呼叫來源）。
- 診斷增強不進行 silent migration：不會主動清空/刪除 `GEMINI_*`、`OPENAI_*`、`ANTHROPIC_*`、`LITELLM_*` 的歷史值，僅在可用性診斷上提示。

### 官方相容來源（用於排障核對）

- LiteLLM 官網：<https://docs.litellm.ai/>
- LiteLLM OpenAI Compatible 說明：<https://docs.litellm.ai/docs/providers/openai_compatible>
- OpenAI Chat API：<https://platform.openai.com/docs/api-reference/chat>
- DeepSeek API 文件：<https://api-docs.deepseek.com/>
- Kimi Moonshot 相容說明：<https://platform.moonshot.ai/docs/guide/compatibility>
- Gemini OpenAI 相容說明：<https://ai.google.dev/gemini-api/docs/openai>
