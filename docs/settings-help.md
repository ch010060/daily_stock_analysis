...
- `WEBUI_HOST`、`WEBUI_PORT`、`DSA_PUBLIC_HOST`、`DSA_ALLOWED_HOSTS`：監聽地址、埠與 LAN allowlist 只在程序啟動時繫結，儲存後必須重啟當前程序、Docker 容器或服務管理器才會生效；`WEBUI_HOST=0.0.0.0` 必須搭配 `DSA_ALLOW_EXTERNAL_NETWORK=true`，系統只接受 localhost、private LAN IP 或明確列出的 LAN host。
