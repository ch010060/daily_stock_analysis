...
4. 輸入你需要的啟動命令，例如：
    ...
    - 僅啟動 FastAPI：`python main.py --serve-only --host 127.0.0.1 --port 8000`
    - 啟動定時任務：`python main.py --schedule`
5. 點選「儲存」
6. 重啟服務

> Route B 目前以本地/內網安全 profile 為主；若需公開部署場景（`0.0.0.0`/公網監聽）需使用另行定義的部署 profile 並透過獨立稽核。
...
| 變數名 | 說明 |
|--------|------|
| `ANSPIRE_API_KEYS` | Anspire Open API 金鑰（大模型與搜尋共用，推薦） |
| `AIHUBMIX_KEY` | AIHubMix API 金鑰（一 Key 多模型，推薦） |
| `GEMINI_API_KEY` | Gemini API 金鑰 |
| `OPENAI_API_KEY` | OpenAI 相容 API 金鑰 |
| `SERPAPI_API_KEYS` | SerpAPI 金鑰（推薦） |
| `TAVILY_API_KEYS` | Tavily API 金鑰（用逗號分隔） |
| `BOCHA_API_KEYS` | Bocha API 金鑰（用逗號分隔） |
| `BRAVE_API_KEYS` | Brave Search API 金鑰（用逗號分隔） |
| `MINIMAX_API_KEYS` | MiniMax API 金鑰（用逗號分隔） |
| `SEARXNG_BASE_URLS` | SearXNG 例項地址（逗號分隔，無配額兜底，需在 settings.yml 啟用 format: json）；server-safe/local-only 模式僅允許 loopback 例項 |
| `SEARXNG_PUBLIC_INSTANCES_ENABLED` | 是否在 `SEARXNG_BASE_URLS` 為空時自動從 `searx.space` 獲取公共例項（預設 `false`，fail-closed） |
