...
DISCORD_INTERACTIONS_PUBLIC_KEY=your-public-key (僅接收入站 Interaction/Webhook 回撥時需要)
DISCORD_BOT_STATUS=A股智慧分析 | /help
```

如果你配置了 Discord Interaction / Webhook 入站回撥，務必在 Discord Developer Portal 的 `General Information -> Public Key` 複製公鑰並填入 `DISCORD_INTERACTIONS_PUBLIC_KEY`；系統會使用該公鑰校驗每個入站請求的 Ed25519 簽名，驗籤失敗會直接拒絕請求。
