...
> Route B server-safe profile 当前不建议在本 phase 开启公开监听。
> 仅支持本机回环绑定（127.0.0.1）；如需公网可见，属于另一个专门部署档（未在本 PR 内打开）。
...
`127.0.0.1` 为本机回环绑定，保持该配置。

> Route B 在当前 gate 修訂下默认走本機安全模式，`0.0.0.0` 公网监听会触发本地安全閘道拒絕。
...
项目的 `docker/docker-compose.yml` 在容器内部保持 `WEBUI_HOST=127.0.0.1`。
...
3. 方向选「入方向」，端口范围填 `8000/8000`，授权对象按你的网络边界策略配置（当前文档不支持 Route B 的公网监听）。
...
这是第二常见原因。`.env` 里默认是 `WEBUI_HOST=127.0.0.1`，这样服务只监听本机。

在 Route B server-safe profile 下这是预期行为；如需变更为公开监听，请走“公开部署”专用配置。
