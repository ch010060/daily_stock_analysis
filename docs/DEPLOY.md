...
Docker 映象啟動入口會自動建立並修復 `./data`、`./logs`、`./reports` 對應掛載目錄的許可權，然後降權為非 root 使用者 (`dsa`, UID 1000) 執行應用。普通部署不需要手動 `chown` / `chmod`。

如果你顯式指定了 `--user` / Compose `user:`，或使用只讀掛載、rootless Docker、NFS 等不允許容器修復屬主的環境，請確保實際執行使用者對這些目錄具備寫入許可權。
...
# 啟動 Web 管理介面（本地/內網安全模式：保持 WEBUI_HOST=127.0.0.1）
...
| 配置項 | 說明 | 獲取方式 |
|--------|------|----------|
| `ANSPIRE_API_KEYS` / `AIHUBMIX_KEY` / `GEMINI_API_KEY` / `ANTHROPIC_API_KEY` / `OPENAI_API_KEY` | AI 模型至少配置一個；推薦優先 Anspire 或 AIHubMix | 對應服務商控制檯 |
| `STOCK_LIST` | 自選股列表 | 逗號分隔的股票程式碼 |
| 通知通道 | 至少配置一個，如企業微信、飛書、Telegram 或郵件 | 對應通知平臺 |
...
| `ANSPIRE_API_KEYS` | - | Anspire 大模型與新聞搜尋（推薦） |
| `AIHUBMIX_KEY` | - | AIHubMix 一 Key 多模型（推薦） |
| `SERPAPI_API_KEYS` | - | SerpAPI 實時金融新聞搜尋（推薦） |
| `TAVILY_API_KEYS` | - | Tavily 新聞搜尋（可選） |
...
### 5. WebUI 開啟後 UI 元素異常變大 / 佈局錯亂

**症狀**：能訪問 8000 埠，但頁面上的文字、按鈕、卡片異常放大，沒有正常佈局。

**根因**：`static/index.html` 存在，但 CSS/JS 資原始檔缺失（`static/assets/` 為空或不存在），瀏覽器無法載入樣式與指令碼，導致裸 HTML 渲染。

**解決方法**：

- **Docker 部署**：執行以下命令重新構建映象（確保前端已正確打包進映象）：
  ```bash
  docker-compose -f ./docker/docker-compose.yml down
  docker-compose -f ./docker/docker-compose.yml build --no-cache
  docker-compose -f ./docker/docker-compose.yml up -d
  ```
  構建完成後重新整理瀏覽器快取（`Ctrl+Shift+R`）再訪問。

- **直接部署（pip + python）**：先構建前端，再啟動服務：
  ```bash
  # 安裝 Node.js 18+（推薦 20+，如尚未安裝）
  # 構建前端
  cd apps/dsa-web
  npm ci
  npm run build
...
**驗證**：用瀏覽器開發者工具（F12 → Network）檢查是否有 `/assets/index-*.js` 和 `/assets/index-*.css` 的 404 錯誤；如有，說明資源缺失，按上述步驟重新構建即可。
...
| Secret 名稱 | 說明 | 必填 |
|------------|------|------|
| `ANSPIRE_API_KEYS` | Anspire Open API Key（一 Key 啟用大模型與搜尋） | 推薦 |
| `AIHUBMIX_KEY` | AIHubMix API Key（一 Key 多模型） | 推薦 |
...
> *注：通知通道至少配置一個，支援多通道同時推送

#### 3. 驗證 Workflow 檔案

確保 `.github/workflows/00-daily-analysis.yml` 檔案存在且已提交：
...
修改時間：編輯 `.github/workflows/00-daily-analysis.yml` 中的 cron 表示式：
...
**祝部署順利！🎉**
> Route B server-safe profile 的 `main.py` 與 Docker entry 都是本地偵聽優先；公開雲端/容器釋出請使用獨立的公開部署檔並單獨評估安全放行策略。
