...
class TushareFetcher(BaseFetcher):
    ...
    def _init_api(self) -> None:
        ...
        這裡直接使用內建 HTTP client，避免執行時強依賴 tushare SDK，
        從而減少 Docker / PyInstaller / 多虛擬環境場景下因缺包導致的初始化失敗。
        """
        config = get_config()
    ...
    def _convert_stock_code(self, stock_code: str) -> str:
        """
        轉換 A 股 / ETF / 北交所等為 Tushare ts_code（不含港股邏輯）。
        ...
        Returns:
            Tushare 格式程式碼，如 '600519.SH', '000001.SZ'
        """
        ...
        if _is_us_code(raw_code):
            raise DataFetchError(f"TushareFetcher 不支援美股 {raw_code}，請使用 AkshareFetcher 或 YfinanceFetcher")

        if _is_hk_market(raw_code):
            #raise DataFetchError(f"TushareFetcher 不支援港股 {raw_code}，請使用 AkshareFetcher")
    ...
    def _convert_hk_stock_code_for_tushare(self, stock_code: str) -> str:
        """
        將使用者輸入轉為 Tushare Pro 介面所需的 ts_code（含港股 nnnnn.HK）。

        - 非港股：委託 _convert_stock_code（A 股 / ETF / 北交所等）。
        - 港股：從 HK00700、00700、00700.HK 等形式歸一為 5 位數字 + .HK。
        """
        raw_code = stock_code.strip()
        if _is_hk_market(raw_code):
            if "." in raw_code:
            ...
            if not digits:
                raise DataFetchError(f"無法識別港股程式碼 {raw_code}")
    ...
    def _normalize_data(self, df: pd.DataFrame, stock_code: str) -> pd.DataFrame:
        ...
        單位縮放僅適用於 A 股（及 ETF 等使用同一套單位的介面）：
        - vol 按「手」計，乘以 100 轉為「股」
        - amount 按「千元」計，乘以 1000 轉為「元」

        港股 hk_daily 返回的 vol / amount 已是可直接使用的量級，不做上述縮放。
        """
        df = df.copy()
        is_hk = _is_hk_market(stock_code)
        ...
        # 成交量 / 成交額：僅 A 股類介面做單位換算（港股 hk_daily 不換算）
    ...
    def get_stock_list(self) -> Optional[pd.DataFrame]:
        ...
        Returns:
            包含 code, name, industry, area, market 列的 DataFrame，失敗返回 None
        """
        if self._api is None:
    ...
    def get_chip_distribution(self, stock_code: str) -> Optional[ChipDistribution]:
        ...
        注意：ETF/指數沒有籌碼分佈資料，會直接返回 None；港股不支援，直接返回 None。
        ...
        if _is_hk_market(stock_code):
            logger.warning(f"[Tushare] TushareFetcher 不支援港股 {stock_code} 的籌碼分佈")
            return None
