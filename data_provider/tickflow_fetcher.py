...
_CN_MAIN_INDEX_QUOTES = (
    ("000001.SH", "000001", "上證指數"),
    ("399001.SZ", "399001", "深證成指"),
    ("399006.SZ", "399006", "創業板指"),
    ("000688.SH", "000688", "科創50"),
    ("000016.SH", "000016", "上證50"),
    ("000300.SH", "000300", "滬深300"),
)
...
class TickFlowFetcher(BaseFetcher):
    ...
    def close(self) -> None:
        ...
        if client is not None:
            try:
                ...
                logger.debug("[TickFlowFetcher] 關閉客戶端失敗: %s", exc)
    ...
    def get_main_indices(self, region: str = "cn") -> Optional[List[Dict[str, Any]]]:
        ...
        if not quotes:
            logger.warning("[TickFlowFetcher] 指數行情為空")
            return None
    ...
    def get_market_stats(self) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if self._is_universe_permission_error(exc):
                ...
                self._universe_query_checked_at = now
                logger.info(
                    "[TickFlowFetcher] 當前套餐不支援標的池查詢，市場統計回退到現有資料來源"
                )
                return None
            raise
        if not quotes:
            logger.warning("[TickFlowFetcher] 市場統計行情為空")
            return None
        ...
        for quote in quotes:
            ...
            if not name:
                logger.debug("[TickFlowFetcher] 缺少股票名稱，按非 ST 處理: %s", symbol)
        ...
        if valid_rows == 0:
            logger.warning("[TickFlowFetcher] 市場統計未命中有效 A 股行情")
            return None
