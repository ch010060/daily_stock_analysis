...
"""
===================================
LongbridgeFetcher - 長橋兜底資料來源 (Priority 5)
===================================

資料來源：長橋 OpenAPI (https://open.longbridge.com)
特點：覆蓋美股 + 港股，可計算量比/換手率/PE 等 yfinance 缺失欄位
定位：美股/港股最後兜底資料來源

關鍵策略：
1. 組合 quote + static_info 介面計算 turnover_rate / pe_ratio / total_mv
2. 透過 history_candlesticks 計算 volume_ratio（近5日均量比）
3. 懶載入 QuoteContext，首次呼叫時才建立連線
4. static_info 程序內短快取，減少重複請求（預設 24h，可調；見 LONGBRIDGE_STATIC_INFO_TTL_SECONDS）

憑證：優先使用 `LONGBRIDGE_OAUTH_CLIENT_ID` + SDK token 快取（OAuth 2.0）；
Legacy API Key 三件套（`LONGBRIDGE_APP_KEY` / `LONGBRIDGE_APP_SECRET` / `LONGBRIDGE_ACCESS_TOKEN`）仍相容。
可選：`LONGBRIDGE_STATIC_INFO_TTL_SECONDS`；SDK `language` 取自 `REPORT_LANGUAGE`，`log_path` 為 `{LOG_DIR}/longbridge_sdk.log`；
`LONGBRIDGE_HTTP_URL` / `LONGBRIDGE_QUOTE_WS_URL` / `LONGBRIDGE_TRADE_WS_URL` / `LONGBRIDGE_REGION` （見官方文件預設值）。
"""
...
def _sanitize_longbridge_env() -> None:
    ...
    if not os.environ.get("LONGBRIDGE_LOG_PATH"):
        try:
            ...
            logger.debug("[Longbridge] 設定 LONGBRIDGE_LOG_PATH=%s",
    ...
    if region:
        ...
        for env_name, default_url in (
            ...
            if default_url and not os.environ.get(env_name):
                ...
                logger.debug("[Longbridge] 根據 REGION=%s 設定 %s=%s",
...
def _restore_oauth_token_cache_from_env(client_id: str) -> bool:
    ...
    try:
        ...
        logger.warning("[Longbridge] OAuth token cache base64 解碼失敗: %s", exc)
        return False

    if not payload:
        logger.warning("[Longbridge] OAuth token cache base64 為空，跳過恢復")
        return False
    ...
    if token_cache.exists():
        try:
            if token_cache.read_bytes() == payload:
                logger.debug("[Longbridge] OAuth token 快取已與 env secret 一致，跳過恢復: %s", token_cache)
                return False
        except OSError as exc:
            logger.warning("[Longbridge] 讀取現有 OAuth token 快取失敗，將嘗試用 env secret 覆蓋: %s", exc)

    try:
        token_cache.parent.mkdir(parents=True, exist_ok=True)
        token_cache.write_bytes(payload)
        token_cache.chmod(0o600)
        logger.info("[Longbridge] 已從 LONGBRIDGE_OAUTH_TOKEN_CACHE_B64 恢復 OAuth token 快取")
        return True
    except Exception as exc:
        logger.warning("[Longbridge] 寫入 OAuth token 快取失敗: %s", exc)
        return False
...
def _is_valid_oauth_cache_file(token_cache: Path) -> bool:
    ...
    try:
        ...
        logger.warning("[Longbridge] 讀取 OAuth token 快取失敗: %s", exc)
        return False

    if not raw.strip():
        logger.warning("[Longbridge] OAuth token 快取為空檔案: %s", token_cache)
        return False

    try:
        payload = raw.decode("utf-8").strip()
    except UnicodeDecodeError as exc:
        logger.warning("[Longbridge] OAuth token 快取不是 UTF-8 文字: %s", exc)
        return False

    try:
        data = json.loads(payload)
    except json.JSONDecodeError as exc:
        logger.warning("[Longbridge] OAuth token 快取不是合法 JSON: %s", exc)
        return False

    if not isinstance(data, dict) or not data:
        logger.warning("[Longbridge] OAuth token 快取內容為空或格式不符合預期: %s", token_cache)
        return False
...
def _oauth_reauth_not_supported(url: str) -> None:
    raise RuntimeError(
        f"OAuth token 快取已失效或缺失，當前為無頭執行不支援開啟授權頁面，請重建 LONGBRIDGE_OAUTH_TOKEN_CACHE_B64: {url}"
    )


def _oauth_sdk_unavailable_error() -> RuntimeError:
    return RuntimeError(
        "當前安裝的 longbridge SDK 不支援 OAuth 2.0（缺少 OAuthBuilder/Config.from_oauth）。"
        "請在支援該 SDK 版本的平臺安裝 longbridge>=4.0.0，或繼續使用 Legacy 三件套。"
    )
...
class LongbridgeFetcher(BaseFetcher):
    ...
    優先順序: 5（最低，作為美股/港股最後兜底）
    資料來源: Longbridge OpenAPI

    透過組合多個 API 計算 yfinance 缺失的指標:
    ...
    def _mark_connection_cooldown(self, exc: Exception) -> None:
        ...
        logger.warning(
            "[Longbridge] 檢測到連線異常，進入 %ss 冷卻期以避免頻繁重連: %s",
            cooldown_seconds,
            exc,
        )
    ...
    def is_available_for_request(self, capability: str = "") -> bool:
        ...
        if self._cooldown_until > time.time():
            logger.debug(
                "[Longbridge] %s 冷卻中，暫時跳過請求，剩餘 %.1fs",
        ...
        with self._ctx_lock:
            ...
            try:
                ...
                if oauth_client_id:
                    ...
                    if _is_valid_oauth_cache_file(token_cache):
                        try:
                            ...
                            logger.info("[Longbridge] Config.from_oauth() 建立成功")
                        except (ImportError, AttributeError):
                            oauth_error = _oauth_sdk_unavailable_error()
                            logger.warning("[Longbridge] OAuth SDK 不可用: %s", oauth_error)
                        except Exception as exc:
                            oauth_error = exc
                            logger.warning("[Longbridge] OAuth 初始化失敗: %s", exc)
                    elif token_cache.exists():
                        logger.warning(
                            "[Longbridge] OAuth token 快取內容異常，已拒絕互動式續期: %s。"
                            "請先執行 scripts/generate_longbridge_oauth_token.py 重建快取。",
                            token_cache,
                        )
                    else:
                        logger.warning(
                            "[Longbridge] OAuth client 已配置，但 token 快取不存在: %s。"
                            "請先執行 scripts/generate_longbridge_oauth_token.py 生成快取；"
                ...
                if lb_config is None and has_legacy:
                    ...
                    for factory_name in ("from_apikey_env", "from_env"):
                        ...
                        try:
                            ...
                            logger.debug(
                                "[Longbridge] Config.%s() 失敗: %s", factory_name, e
                            )
                ...
                if lb_config is None and has_legacy:
                    ...
                    reason = (
                        f"OAuth 初始化失敗: {oauth_error}"
                        if oauth_error
                        else "未找到可用 OAuth token 快取且未配置完整 Legacy 三件套"
                    )
                    logger.warning("[Longbridge] 未建立認證配置: %s", reason)
                ...
                logger.warning("[Longbridge] QuoteContext 初始化失敗: %s", e)
                self._available = False
                return None
    ...
    def _get_static_info(self, symbol: str) -> Optional[Any]:
        ...
        try:
            ...
            logger.debug(f"[Longbridge] static_info({symbol}) 失敗: {e}")
    ...
    def _compute_volume_ratio(self, symbol: str, today_volume: int) -> Optional[float]:
        ...
        try:
            ...
            logger.debug(f"[Longbridge] 計算量比失敗({symbol}): {e}")
            return None
    ...
    def get_realtime_quote(self, stock_code: str) -> Optional[UnifiedRealtimeQuote]:
        ...
        symbol = _to_longbridge_symbol(stock_code)
        if symbol is None:
            logger.debug(f"[Longbridge] 無法轉換程式碼: {stock_code}")
            return None
        ...
        try:
            ...
            logger.info(f"[Longbridge] quote({symbol}) 失敗: {e}")
            if self._is_connection_error(e):
                self._mark_connection_cooldown(e)
            return None
        ...
        if static is not None:
            ...
            if shares_for_turnover > 0 and volume > 0:
                ...
                logger.debug(
                    "[Longbridge] %s 無法計算換手率: volume=%s circulating=%s total_shares=%s",
                    symbol,
                    volume,
                    circulating,
                    total_shares,
        ...
        logger.info(
            ...
            f"價格={price}, 量比={volume_ratio}, 換手率={turnover_rate}"
        )
        return quote
