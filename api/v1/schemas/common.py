...
class RootResponse(BaseModel):
    ...
    message: str = Field(..., description="API 執行狀態訊息", json_schema_extra={"example": "Daily Stock Analysis API is running"})
...
class HealthResponse(BaseModel):
    ...
    status: str = Field(..., description="服務狀態", json_schema_extra={"example": "ok"})
...
class ErrorResponse(BaseModel):
    ...
    error: str = Field(..., description="錯誤型別", json_schema_extra={"example": "validation_error"})
    message: str = Field(..., description="錯誤詳情", json_schema_extra={"example": "請求引數錯誤"})
