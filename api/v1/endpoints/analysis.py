...
def trigger_analysis(
    ...
    if len(stock_codes) > MAX_BATCH_SIZE:
        raise HTTPException(
            ...
            detail={
                "error": "validation_error",
                "message": f"單次分析請求最多支援 {MAX_BATCH_SIZE} 只股票"
            }
        )
        ...
        if rejected:
            raise HTTPException(
                ...
                detail={
                    ...
                    "message": f"Route B 模式下不允許提交以下程式碼：{', '.join(rejected)}。僅支援 TW/US 市場。",
...
def _handle_sync_analysis(
    ...
    try:
        ...
        if result is None:
            error_message = service.last_error or f"分析股票 {stock_code} 失敗"
            raise HTTPException(
                status_code=500,
                detail={
