...
export function parseApiError(error: unknown): ParsedApiError {
  ...
  if (errorCode === 'portfolio_oversell' || includesAny(matchText, ['oversell detected'])) {
    return createParsedApiError({
      title: '賣出數量超過可用持股',
      message: '賣出數量超過當前可用持股，請刪除或修正對應賣出流水後重試。',
      rawMessage,
      status,
  ...
  if (errorCode === 'alphasift_install_failed') {
    return createParsedApiError({
      title: 'AlphaSift 自動安裝失敗',
      message: 'DSA 已嘗試自動安裝 AlphaSift，但 pip 安裝未成功。請檢查 ALPHASIFT_INSTALL_SPEC、網路代理或後端 Python 環境。',
      rawMessage,
      status,
      category: 'http_error',
    });
  ...
  if (errorCode === 'alphasift_install_spec_missing') {
    return createParsedApiError({
      title: 'AlphaSift 安裝來源未配置',
      message: '請先在設定頁把 ALPHASIFT_INSTALL_SPEC 配置為受信任的 AlphaSift GitHub 倉庫，再開啟選股。',
      rawMessage,
      status,
      category: 'http_error',
    });
  ...
  if (errorCode === 'alphasift_install_spec_not_allowed') {
    return createParsedApiError({
      title: 'AlphaSift 安裝來源受限',
      message: '自動安裝僅允許使用受信任的 AlphaSift GitHub 來源；如需本地路徑或 wheel，請先手動安裝到當前 Python 環境。',
      rawMessage,
      status,
      category: 'http_error',
    });
  ...
  if (errorCode === 'alphasift_unavailable' || includesAny(matchText, ['cannot import alphasift', 'alphasift.screen'])) {
    return createParsedApiError({
      title: 'AlphaSift 未就緒',
      message: '當前 DSA 後端環境無法匯入 alphasift。請先安裝或掛載 AlphaSift，例如在同一個 Python 環境執行 python -m pip install -e /path/to/alphasift。',
      rawMessage,
      status,
      category: 'http_error',
    });
  ...
  if (errorCode === 'alphasift_adapter_unavailable') {
    return createParsedApiError({
      ...
      message: '當前 AlphaSift 版本缺少 DSA 穩定適配層。請重新安裝或升級 AlphaSift 後再試。',
      category: 'http_error',
      rawMessage,
      status,
    });
