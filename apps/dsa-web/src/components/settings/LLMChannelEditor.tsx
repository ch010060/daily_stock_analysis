        ...
        <div className="settings-surface-overlay-soft space-y-4 px-4 py-4">
          ...
          <div className="space-y-3 rounded-xl border border-[var(--settings-border)] bg-[var(--settings-surface-hover)] p-3">
            <div className="flex flex-wrap items-center gap-2">
              ...
              >
                {discoveryState?.text || '支援 `/models` 的 OpenAI Compatible 通道可自動拉取模型。'}
              </span>
            </div>
            {discoveryState?.hint ? (
...
const LLM_TROUBLESHOOTING_HINTS: Record<string, string> = {
  auth: '請檢查 API Key 是否正確、是否有多餘空格，以及當前通道是否需要額外組織/專案許可權。',
  timeout: '可重試；若持續超時，請檢查 Base URL、網路代理、服務商可用區或本地防火牆。',
  quota: '請檢查餘額、套餐額度、RPM/TPM 限流或併發設定，必要時稍後重試。',
  model_not_found: '請確認模型名與通道協議匹配，並先用“獲取模型”核對該通道實際可用模型列表。',
  empty_response: '通道已連通但未返回正文；可嘗試切換相容模型、關閉額外響應模式後再測試。',
  network_error: '請檢查 Base URL、代理、TLS/證書、中轉閘道器或本地網路策略，並可稍後重試。',
  invalid_config: '先補齊協議、Base URL、API Key 和模型配置，再執行一鍵測試。',
  unsupported_protocol: '當前僅對 OpenAI Compatible / DeepSeek 通道提供自動模型發現，請改為手動維護模型列表。',
};
...
const LLM_REASON_HINTS: Record<string, string> = {
  ...
  insufficient_balance: '服務商返回餘額、賬單或額度不足；請檢查帳戶餘額和套餐狀態。',
  ...
  provider_prefix_mismatch: '模型 provider 字首與當前通道不匹配；請確認模型名是否應使用該通道的 OpenAI-compatible 路由。',
  capability_unsupported: '當前模型或相容層不支援該能力；這不影響基礎文字連線，可換模型或關閉該能力依賴。',
};
...
function getLlmTroubleshootingHint(
  ...
  if (code === 'format_error') {
    return context === 'discovery' || stage === 'model_discovery'
      ? '該通道返回的 /models 響應格式不相容，請改為手動填寫模型列表。'
      : '返回結構與預期不一致，請確認該通道相容 Chat Completions 介面。';
  }
  if (code === 'empty_response' && (context === 'discovery' || stage === 'model_discovery')) {
    return '該通道的 /models 介面未返回可用模型 ID；請檢查 Base URL 是否指向相容的模型列表介面，或改為手動填寫模型列表。';
  }
...
function summarizeCapabilityResults(results: Partial<Record<LLMCapabilityCheck, LLMCapabilityCheckResult>>): string {
  ...
  return `能力檢測完成：${passed} 透過 / ${failed} 失敗 / ${skipped} 跳過`;
}
...
export const LLMChannelEditor: React.FC<LLMChannelEditorProps> = ({
  ...
  const handleSave = async () => {
    ...
    if (hasEmptyName) {
      setSaveMessage({ type: 'local-error', text: '通道名稱不能為空，且只能包含字母、數字或下劃線。' });
      return;
    }
    ...
    if (managesRuntimeConfig) {
      ...
      if (invalidPrimaryModel) {
        setSaveMessage({ type: 'local-error', text: '當前主模型不在已啟用通道的模型列表中，請重新選擇。' });
        return;
      }
      ...
      if (invalidAgentPrimaryModel) {
        setSaveMessage({ type: 'local-error', text: '當前 Agent 主模型不在已啟用通道的模型列表中，請重新選擇。' });
        return;
      }
      ...
      if (invalidVisionModel) {
        setSaveMessage({ type: 'local-error', text: '當前 Vision 模型不在已啟用通道的模型列表中，請重新選擇。' });
        return;
      }
    }
    ...
    try {
      ...
      setSaveMessage({ type: 'success', text: managesRuntimeConfig ? 'AI 配置已儲存' : '通道配置已儲存' });
    } catch (error: unknown) {
      setSaveWarnings([]);
      ...
      >
        <div className="space-y-1">
          <div className="flex items-center gap-2">
            ...
            <Badge variant="info" className="settings-accent-badge">通道管理</Badge>
          </div>
          <p className="text-xs text-muted-text">
            新增服務商通道後可自動獲取模型列表並多選，也可繼續手動填寫。配置會自動同步到 .env 檔案。
          </p>
        </div>
        ...
        <div className="space-y-4 animate-in fade-in slide-in-from-top-2 duration-300">
          <div className="rounded-[1.35rem] border border-[var(--settings-border)] bg-[var(--settings-surface)] p-4 shadow-soft-card">
            <div className="mb-3 flex items-center justify-between">
              <div>
                <h4 className="text-sm font-medium text-foreground">快速新增通道</h4>
                <p className="mt-1 text-xs text-secondary-text">先選擇預設服務商，再一鍵建立配置草稿。</p>
              </div>
              <Badge variant="default" className="border-[var(--settings-border)] bg-[var(--settings-surface-hover)] text-muted-text">{channels.length} 個通道</Badge>
            </div>
            <div className="flex items-center gap-2">
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between px-1">
              <span className="text-xs font-medium uppercase tracking-wider text-muted-text">通道列表</span>
              {channels.length > 0 ? (
            ...
            {channels.length === 0 ? (
              <div className="settings-surface-overlay-muted rounded-[1.35rem] border border-dashed settings-border-strong px-4 py-10 text-center">
                <p className="text-sm font-medium text-secondary-text">還沒有通道</p>
                <p className="mt-1 text-xs text-muted-text">選擇服務商預設後點選“新增通道”即可開始配置。</p>
              </div>
          ...
          {managesRuntimeConfig ? (
            <div className="rounded-[1.35rem] border border-[var(--settings-border)] bg-[var(--settings-surface)] p-4 shadow-soft-card">
              ...
              {availableModels.length === 0 ? (
                <div className="rounded-xl border border-dashed settings-border-strong settings-surface-overlay-soft px-3 py-2 text-xs text-muted-text">
                  先新增至少一個已啟用通道並填寫模型，下面的主模型 / 備選模型 / Vision 選項才會出現。
                </div>
              ) : (
                <div className="space-y-4">
                  <div>
            ...
            <InlineAlert
              variant="warning"
              message="檢測到已配置高階模型路由 YAML：此處僅管理通道條目和基礎連線資訊。執行時主模型 / 備選模型 / Vision / Temperature 仍由下方通用欄位決定；若 YAML 解析成功，則以其中的路由與可用模型宣告為準，本配置不會覆蓋 YAML 檔案本身。"
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            >
              {isSaving ? '儲存中...' : managesRuntimeConfig ? '儲存 AI 配置' : '儲存通道配置'}
            </Button>
