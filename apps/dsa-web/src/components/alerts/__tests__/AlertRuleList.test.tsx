...
describe('AlertRuleList', () => {
  ...
  it('renders rules, filters, and pagination', () => {
    ...
    fireEvent.change(screen.getByLabelText('啟停狀態'), { target: { value: 'enabled' } });
    fireEvent.change(screen.getByLabelText('規則型別'), { target: { value: 'price_cross' } });
  ...
  it('renders portfolio scope labels and child-target cooldown hint', () => {
    ...
    expect(screen.getByText('持股標的')).toBeInTheDocument();
    expect(screen.getByText('子目標見觸發歷史')).toBeInTheDocument();
    expect(screen.getByText('帳戶 9')).toBeInTheDocument();
  ...
  it('shows an empty state for no rules', () => {
    ...
    expect(screen.getByText('暫無警告規則')).toBeInTheDocument();
  });
});
