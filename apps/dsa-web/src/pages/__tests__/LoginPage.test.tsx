...
describe('LoginPage', () => {
  ...
  it('blocks first-time setup when confirmation does not match', async () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: '完成設定並登入' }));
  ...
  it('navigates to redirect after a successful login', async () => {
    ...
    fireEvent.change(screen.getByLabelText('登入密碼'), { target: { value: 'passwd6' } });
    ...
    expect(screen.getByLabelText('登入密碼')).toHaveAttribute('data-appearance', 'login');
  });
