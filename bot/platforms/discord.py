...
class DiscordPlatform(BotPlatform):
    ...
    def verify_request(self, headers: Dict[str, str], body: bytes) -> bool:
        ...
        if not signature or not timestamp:
            logger.warning("[Discord] 缺少簽名頭，拒絕請求")
            return False

        # 校驗 timestamp 格式與時效性，防止重放攻擊
        try:
            ts_int = int(timestamp)
        except (TypeError, ValueError):
            logger.warning("[Discord] 非法的 timestamp：必須為 Unix 秒整數，拒絕請求")
            return False

        try:
            now_ts = int(time.time())
        except Exception as exc:
            logger.warning("[Discord] 獲取當前時間失敗: %s，拒絕請求", exc)
            return False

        # 允許的時間視窗：±5 分鐘
        if abs(now_ts - ts_int) > 300:
            logger.warning(
                "[Discord] 請求 timestamp 超出允許視窗，可能為重放攻擊：timestamp=%s, now=%s",
                ts_int,
                now_ts,
            )
            return False
        ...
        try:
            ...
            logger.warning("[Discord] 公鑰或簽名不是合法十六進位制，拒絕請求")
            return False
        except Exception as exc:
            logger.warning("[Discord] 無法載入簽名公鑰: %s", exc)
            return False

        try:
            verify_key.verify(timestamp.encode("utf-8") + body, signature_bytes)
        except BadSignatureError:
            logger.warning("[Discord] 簽名驗證失敗")
            return False
        except Exception as exc:
            logger.warning("[Discord] 簽名校驗異常: %s", exc)
            return False
    ...
    def handle_webhook(
        ...
        """Discord 需要先驗籤，再處理 ping/challenge。"""
    ...
    def format_response(self, response: Any, message: BotMessage) -> WebhookResponse:
        ...
        對於 Interaction（type=2）請求，返回 Discord Interaction Response
        ...
        # Interaction（slash-command）需要 Interaction Response 回撥格式
    ...
    def send_followup(self, response: Any, message: BotMessage) -> bool:
        ...
        if not application_id or not interaction_token:
            logger.warning(
                "[Discord] 缺少 application_id 或 interaction token，無法傳送 follow-up"
            )
            return False
        ...
        try:
            ...
            logger.warning("[Discord] 訊息分塊失敗: %s，嘗試整段傳送", exc)
            chunks = [content]
        ...
        for idx, chunk in enumerate(chunks):
            try:
                ...
                if resp.status_code >= 300:
                    logger.error(
                        "[Discord] follow-up chunk %d/%d 傳送失敗: %s %s",
                        idx + 1,
                        len(chunks),
                        resp.status_code,
                        resp.text[:200],
                ...
                logger.error(
                    "[Discord] follow-up chunk %d/%d 請求異常: %s",
                    idx + 1,
                    len(chunks),
                    exc,
                )
        ...
        if success:
            logger.info("[Discord] follow-up 訊息傳送成功 (%d 塊)", len(chunks))
        return success
