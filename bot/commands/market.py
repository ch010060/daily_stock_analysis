...
class MarketCommand(BotCommand):
    ...
    def execute(self, message: BotMessage, args: List[str]) -> BotResponse:
        ...
        if lock_token is None:
            return BotResponse.markdown_response("⚠️ 大盤覆盤正在執行中，請稍後再試。")
        ...
        try:
            ...
            logger.error(
                "[MarketCommand] 大盤覆盤後臺執行緒啟動失敗: %s",
                exc,
            )
            self._release_market_review_lock(lock_token)
            return BotResponse.error_response(
                "大盤覆盤啟動失敗，已釋放執行鎖；請稍後重試"
            )
    ...
    def _run_market_review(
        ...
        try:
            ...
            if override_region == "":
                ...
                logger.info("[MarketCommand] 今日相關市場休市，跳過大盤覆盤")
            ...
            logger.error("[MarketCommand] 大盤覆盤失敗: %s", e)
            logger.exception(e)
        finally:
            self._release_market_review_lock(lock_token)
